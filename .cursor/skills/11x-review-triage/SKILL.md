...
The review skills only report. This is the named, deliberate place where their findings become edits — so fixes never happen silently inside a review. Interactive runs a selection loop (must NOT be isolated in a subagent). Unattended applies a fixed policy instead.
...
- If `change.md` has `mode: unattended`, load `unattended-policy`: do not ask; apply **Critical** only; park Suggestion / Nice-to-have in `handoff.md`.

## Steps

1. Read the latest `context/changes/<id>/reviews/*.md` (plan-review and/or impl-review).
2. Interactive: present findings as a checklist grouped by severity (Critical / Suggestion / Nice-to-have). Ask the user which to apply (default: all Critical). **Unattended:** skip the ask; treat the chosen set as all Critical.
