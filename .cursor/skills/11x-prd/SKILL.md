...
1. Read the notes plus `context/foundation/product-decisions.md` and `product-research/synthesis.md` when present. Settled product decisions take precedence over earlier shaping hypotheses; ask if they conflict. Detect `context_type` (frontmatter, else infer from cwd: an existing codebase → brownfield).
