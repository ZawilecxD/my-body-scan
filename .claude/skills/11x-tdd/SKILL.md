...
2. For each step in the phase, cycle:
   ...
   Mark `- [ ]` → `- [x]` as each step lands. **Unattended:** only Automated steps; never mark Manual `[x]`.
3. **Verify** the phase's Automated Success Criteria. Interactive: surface Manual ones. **Unattended:** append Manual items to `handoff.md` (create if needed); never tick Manual Progress; do not ask.
...
The phase is `[x]` with a SHA (unattended: Automated only), tests are green, and the commit exists. Print the next phase's resume command, or `/11x-impl-review <id>` when the plan is done. Stop.
