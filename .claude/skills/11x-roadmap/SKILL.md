...
- **Foundation mode** — no effort id; reads `context/foundation/prd.md` (+ `tech-stack.md`, `architecture.md` when present, `lessons.md`). Writes `context/foundation/roadmap.md`.
...
4. Write the roadmap: a numbered slice list. For each slice — outcome, layers touched, and a ready-to-paste next command:
   ...
   When composed by Factory, also include testable acceptance criteria, explicit dependencies and stable change IDs for each slice; carry unresolved product decisions back to the user before build approval.
