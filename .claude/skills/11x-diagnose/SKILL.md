...
- If the in-flight change has `mode: unattended`, load `unattended-policy`: never seed a sibling `change.md`. Trivial → inline + regression test. Non-trivial or out of scope → append a follow-up to that change's `handoff.md` (create if needed). If the bug blocks Automated criteria, abort the delivery rather than expanding scope.
...
4. **Fix, then branch on size:**
   ...
   - **Non-trivial (interactive)** → promote it. Write `context/changes/<id>/change.md` (`type: defect`, which turns on the TDD gate in `11x-plan`) and `context/changes/<id>/diagnosis.md` (minimized repro + ranked hypotheses + the regression test) directly — this is what `/11x-new` would do, done inline because a model-invoked skill can't mechanically call a user-invoked one. Then print `/11x-plan <id>` as the next command.
   - **Non-trivial (unattended)** → do not promote. Follow-up on `handoff.md` only.

## Done when

Either the bug is fixed inline with a regression test, or (interactive) a `type: defect` change is seeded with its `diagnosis.md` and `/11x-plan <id>` is printed, or (unattended) a follow-up is on `handoff.md`. Never auto-run the next skill. Stop.
