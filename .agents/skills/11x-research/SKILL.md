...
2. **Pick a mode from the input:**
   - **codebase** (default): fan out read-only explore subagents over facets of the topic if the host has them; otherwise search the tree yourself. One facet ALWAYS searches `context/changes/**/` and `context/archive/**/` for prior decisions on the same topic — if it was researched before, cite that finding instead of re-deriving it.
   - **external** (`--url=` or `--kind=external`): use the host's web fetch/search tools; synthesize with citations and a fetch date. No MCP required.
