...
2. **Fan out** one read-only explore subagent per layer (`frontend`, `backend`, `testing`) if the host has them; otherwise search each layer yourself. Find repeated patterns in actual code.
